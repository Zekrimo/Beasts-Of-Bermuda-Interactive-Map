...
async function start() {
  ...
  const { map: mapConfig, views, groups, categories } = await response.json();
  const { map, setMap } = createMap(mapConfig, views);
  const layers = createMarkerLayers(map, categories);
  setupControls(map, layers, categories, setMap, views, groups);
  addLegendHoverEffects(layers);
}

start().catch(error => {
  console.error(error);
  document.body.insertAdjacentHTML('beforeend', `<p class="map-error">The map could not be loaded: ${error.message}</p>`);
});
